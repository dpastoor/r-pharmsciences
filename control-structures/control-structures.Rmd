...
print (since it is easy to demonstrate) for whether a function prints
...
if statement when the tested condition is **FALSE** you can do-so using the `!` operator, which means *not*, so we are saying we want to run the code when it
