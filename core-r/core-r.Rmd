...
Also of note, is for the package `dplyr`, which will become a key part of one's analysis toolkit,
columns or vectors with additional attributes actually cause errors as dplyr is conservative, and rather than accidentally stripping key attributes before calculations, just errors out, so you must manually remove the additional attributes.
...
* I've also introduced 2 new concepts in the Theoph example, `with` and `within`, using both with and within, scale Theoph's Wt to lbs from kg. Start with the r-examples `?with`. 
